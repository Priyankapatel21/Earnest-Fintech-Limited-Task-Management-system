...
export default function DashboardPage() {
  ...
  return (
    <div className={`flex h-screen transition-all duration-300 ${darkMode ? 'bg-gray-950 text-white' : 'bg-[#FDF9FF] text-gray-900'}`}>
      
      <aside className={`w-64 border-r flex flex-col p-6 hidden md:flex transition-colors ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100 shadow-sm'}`}>
        ...
        <div className="mt-auto px-2 pb-4">
          <div className={`p-4 rounded-2xl border transition-all duration-500 ${darkMode ? 'bg-gray-800/40 border-gray-700' : 'bg-[#A5A2F2]/10 border-[#A5A2F2]/20'}`}>
            ...
            <div className="w-full bg-gray-200/50 h-2 rounded-full overflow-hidden relative">
              <div className={`h-full rounded-full transition-all duration-1000 ease-out shadow-sm ${progressPercent === 100 ? 'bg-green-500' : 'bg-[#5C59C2]'}`} style={{ width: `${progressPercent}%`, boxShadow: progressPercent > 0 ? `0 0 10px ${progressPercent === 100 ? '#22c55e' : '#5C59C2'}44` : 'none' }} />
            </div>
            <p className="text-[9px] font-bold text-gray-400 mt-3 text-center uppercase tracking-tight">
              {progressPercent === 100 ? "🎉 Workspace Cleared" : `${completedCount} of ${tasks.length} tasks completed`}
            </p>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className={`flex justify-between items-center px-8 py-4 transition-colors ${darkMode ? 'bg-gray-900' : 'bg-transparent'}`}>
        ...
        <div className="p-8 overflow-y-auto">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-10">
            ...
            <div className="relative">
              <button onClick={() => { setIsFilterOpen(!isFilterOpen); setIsSortOpen(false); }} className={`flex items-center gap-2 px-5 py-3 border border-gray-200 rounded-xl font-bold ${darkMode ? 'bg-gray-900 text-gray-300 border-gray-800' : 'bg-white text-gray-600'}`}>
                <ListFilter size={18} /> <span>Filter</span> <ChevronDown size={14} className={isFilterOpen ? "rotate-180 transition-transform" : "transition-transform"} />
              </button>
              {isFilterOpen && (
                <div className={`absolute left-0 mt-3 w-48 rounded-2xl shadow-2xl z-[100] border animate-in fade-in slide-in-from-top-2 ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
                  <div className="p-2">
                    {["All", "Work", "Personal", "Shopping", "Health"].map(cat => (
                    <button 
                      key={cat} 
                      onClick={() => {setFilterCategory(cat); setIsFilterOpen(false);}} 
                      className={`w-full text-left px-3 py-2 text-xs font-bold rounded-lg ${filterCategory === cat ? 'bg-[#5C59C2] text-white' : 'hover:bg-[#5C59C2]/10 transition-colors'}`}
                    >
                    {cat}
                    </button>
                    ))}
            ...
            <div className="relative">
              <button onClick={() => { setIsSortOpen(!isSortOpen); setIsFilterOpen(false); }} className={`flex items-center gap-2 px-5 py-3 border border-gray-200 rounded-xl font-bold ${darkMode ? 'bg-gray-900 text-gray-300 border-gray-800' : 'bg-white text-gray-600'}`}>
                <ArrowUpDown size={18} /> <span>Sort By</span> <ChevronDown size={14} className={isSortOpen ? "rotate-180 transition-transform" : "transition-transform"} />
              </button>
              {isSortOpen && (
                <div className={`absolute left-0 mt-3 w-48 rounded-2xl shadow-2xl z-[100] border animate-in fade-in slide-in-from-top-2 ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
                  <div className="p-2">
                    <button onClick={() => {setSortBy("newest"); setIsSortOpen(false);}} className={`w-full text-left px-3 py-2 text-xs font-bold rounded-lg hover:bg-[#5C59C2]/10 transition-colors ${sortBy === "newest" ? "text-[#5C59C2]" : ""}`}>Newest Date</button>
                    <button onClick={() => {setSortBy("oldest"); setIsSortOpen(false);}} className={`w-full text-left px-3 py-2 text-xs font-bold rounded-lg hover:bg-[#5C59C2]/10 transition-colors ${sortBy === "oldest" ? "text-[#5C59C2]" : ""}`}>Oldest Date</button>
                  </div>
                </div>
              )}
            </div>

            <button onClick={() => setShowModal(true)} className="bg-[#5C59C2] text-white px-6 py-3 rounded-xl font-black shadow-lg shadow-[#5C59C2]/30 flex items-center gap-2 hover:bg-[#4B48A3] active:scale-95 transition-all ml-auto">
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
      ...
      <button 
        ...
        className="text-[#5C59C2] font-black text-sm hover:underline underline-offset-4"
      >
